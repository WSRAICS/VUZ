...
crosses_with_out_blocks ={(100, 700+200): {1:10, 2:5}, (300, 500+200): {1: 5, 2:10}, (300, 350+200): {1:5, 2: 10}, (500, 550): {1: 10, 2:0}, (500, 400): {1:10, 4:5}, (300, 400): {1: 10}, (300, 150): {2: 5}, (500, 150): {1: 5, 2:4, 3:6}}
coding = {i: [tuple_name, crosses_with_out_blocks[tuple_name]] for i, tuple_name in enumerate(crosses_with_out_blocks.keys())}
all_coords = [i for i in list(horizontal_storage.keys())] + [i for i in list(up_down_storage.keys())] + [i for i in list(vartical_signs_storage.keys()) + [i for i in list(forks_storage.keys())]]
print(forks)
with open("q_val.txt", "w", encoding="utf-8") as file:
    file.write(json.dumps(coding))
class MainArea():
    def __init__(self, up_down_storage, horizontal_storage, forks_storage, vertical_signs_storage, horizontal_signs_storage, for_q_values, all_coords):
        pg.init()
        self.cars = {1: "static/Wcar.png", 2: "static/WcarRight.png", 3: "static/WcarBottom.png", 4:"static/WcarLeft.png"}
        self.acts = {1: [0, -50], 2: [50, 0], 3: [0, 50], 4: [-50, 0]}
    ...
    def do_step_run(self):
        ...
        reload_Learning_records(time_of_ride=time_ride, number_of_turns=1, is_exit=False)
    def check_best_act(self, i):
        q_val = self.read_q_val()
        best_act = list(dict(sorted(q_val[str(i)][1].items(), key=lambda item: item[1])).keys())[-1]
        return best_act
    ...
    def start_study(self):
        ...
        print(forks_storage)
        self.update()
        print(list(self.forks_storage.keys()))
        for i in range(0, 8):
            print(f"Это i {i}")
            best_act = self.check_best_act(i)
            if best_act_old != best_act:
                n_turns+=1
            best_act_old = best_act
            q_val = self.read_q_val()
            forks = [(i[0][0], i[0][1]) for i in q_val.values()]
            print(forks)
            print(q_val)
            print(best_act)
            if str(self.axis) != best_act:
                self.car = pg.image.load(self.cars[int(best_act)])
                self.x_y = (self.x_y[0] + self.acts[int(best_act)][0], self.x_y[1] + self.acts[int(best_act)][1])
                self.update()
                while (self.x_y[0], self.x_y[1]-200) not in list(self.vertical_signs_storage.keys()) and (self.x_y[0], self.x_y[1]-200) not in forks and (self.x_y[0], self.x_y[1]-200) not in list(self.horizontal_sings_storage.keys()) and (self.x_y[0], self.x_y[1]-200) in self.all_coords:
                    print(self.x_y)
                    self.x_y = (self.x_y[0]+self.acts[int(best_act)][0], self.x_y[1]+self.acts[int(best_act)][1])
                    self.update()
                    # time.sleep(1)
                print(list(self.vertical_signs_storage.keys()))
                if (self.x_y[0], self.x_y[1]-200) in forks:
                    q_val = self.read_q_val()
                    reward = 5
                    ...
                    reload_Learning_records(time_of_ride=time_now, number_of_turns=n_turns, is_exit=False)
                elif (self.x_y[0], self.x_y[1]-200) in list(self.vertical_signs_storage.keys()) or (self.x_y[0], self.x_y[1]-200) in list(self.horizontal_sings_storage.keys()) or (self.x_y[0], self.x_y[1]-200) not in self.all_coords:
                    q_val = self.read_q_val()
                    reward = -5
                    q_val[str(i)][1][best_act] = (1 - self.lr) * q_val[str(i)][1][best_act] + self.lr*reward
                    self.rewrite_q_val(q_val)
                    ...
                    reload_Learning_records(time_of_ride=time_now, number_of_turns=n_turns, is_exit=True)
...
area = MainArea(up_down_storage, horizontal_storage, forks_storage, vartical_signs_storage, horizontal_sings_storage, for_q_values=crosses_with_out_blocks, all_coords=all_coords)
