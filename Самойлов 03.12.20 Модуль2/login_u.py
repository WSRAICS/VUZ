...
def reload_Learning_records(time_of_ride, number_of_turns, is_exit):
    ...
    record = Learning_Records(time_of_ride=time_of_ride, number_of_turns=number_of_turns, is_exit=is_exit)
    session.add(record)
    session.commit()
    session.close()
...
def write_records(time_of_ride, number_of_turns, is_exit):
    ...
    record = Records(time_of_ride=time_of_ride, number_of_turns=number_of_turns, is_exit=is_exit)
    session.add(record)
    session.commit()
    session.close()
...
reload_Learning_records(10, 5, False)
